...
export const storyConfig = {
  ...
  intro: {
    narrativeLines: [
      ...
      'И 1800 километров тишины между нами.',
    ],
  ...
  galaxy: {
    ...
    specialStar: {
      ...
      videoDescription:
        'Этот слой откроется на весь экран и станет самым живым признанием внутри этой галактики.',
      closeLabel: 'Вернуться к звезде',
      continueLabel: 'Видео закончилось. Идти дальше',
      bridgeEyebrow: 'AFTER THE LIGHT',
      bridgeTitle: 'После этого история не заканчивается, а открывается глубже',
      bridgeDescription:
        'Дальше тебя ждет не просто еще один экран, а новая вселенная памяти, в которой каждая звезда хранит для тебя что-то свое.',
      afterVideoTitle: 'Свет уже остался внутри этой галактики',
    ...
    postVideo: {
      ...
      title: 'После света начинается самое личное',
      prefaceLines: [
        'Ты увидела не просто видео — ты перешла порог, за которым экран перестает быть рамкой и становится дверью.',
        'Каждый кадр до этого был лишь тихим набором высоты: теперь ты на высоте, откуда видно целую карту чувств.',
        'Все, что было раньше, подводило сюда не случайно — это был медленный разгон к тому, что я хочу показать лично для тебя.',
        'Дальше — не «ещё один экран», а пространство, где каждая деталь держится на твоём внимании, как звезда на гравитации.',
      ],
      jumpButtonLabel: 'ОБНУЛИТЬ 1800 КМ',
      jumpHoldLabel: 'Удерживай, чтобы запустить гиперпрыжок',
      jumpLines: [
        'Для кода нет границ — а между нами и того меньше: мы уже научились прокладывать траектории там, где другим пишут «нельзя».',
        'Каждый пиксель этой дороги выстроен так, чтобы довезти тебя не до экрана, а до ощущения, что я рядом.',
        'Ты — редкий спектр: тебя не свести к двум цветам; для меня ты вся палитра целиком, без компромиссов.',
        'Если бы любовь была сигналом, ты была бы самой тихой частотой — я бы настроился только на неё и больше ни на что.',
        'Мы не спешим: этот прыжок — как общий выдох, который мы произносим одной фразой, не перебивая друг друга.',
        'Ты — моя личная аномалия: самый тихий отклик на фоне шума и самый яркий свет, когда я зову тебя по имени.',
        'Сейчас мы не «переключаем сцену» — мы стягиваем расстояние до тонкой линии, пока оно не станет почти прикосновением.',
        'Я собираю эту вселенную по крупицам, чтобы в конце пути осталась ты — не идеал из каталога, а просто дом, в который хочется вернуться.',
        'Я держу курс на ту точку, где цифры кончаются, а остаёмся только мы — и я жду нашей встречи уже не на экране, а вживую.',
      ],
      memoryEyebrow: 'COSMOS OF LOVE',
      memoryTitle: 'Родилась вселенная памяти',
      memoryDescription:
        'Это уже не карта расстояния. Это карта того, что между нами выжило, выросло и стало светом.',
      resetTitle: 'Reset Point',
      resetDescription:
        'Эта точка уже видна вдали, но пока еще не дается в руки. До нее нужно дорасти вместе с этим миром.',
      stars: [
        {
          id: 'lumen-trace',
          title: 'Lumen Trace',
          kind: 'message',
          status: 'active',
          teaser: 'Тихий свет, который открывается прикосновением.',
          unlockTitle: 'След света',
          unlockText: 'Здесь живет мягкое послание, которое не кричит, а остается рядом.',
        },
        {
          id: 'velvet-pulse',
          title: 'Velvet Pulse',
          kind: 'message',
          status: 'active',
          teaser: 'Пульс, который отвечает только если задержаться рядом.',
          unlockTitle: 'Бархатный импульс',
          unlockText: 'Эта звезда должна откликаться не на спешку, а на удержание.',
        },
        {
          id: 'mirror-veil',
          title: 'Mirror Veil',
          kind: 'coming-soon',
          status: 'active',
          teaser: 'Скрытый слой, который проявится под правильным углом.',
          unlockTitle: 'Зеркальная вуаль',
          unlockText: 'Эта точка должна открываться через поворот мира и поиск нужного ракурса.',
        },
        {
          id: 'ohana-echo',
          title: 'Ohana Echo',
          kind: 'message',
          status: 'active',
          teaser: 'Там, где однажды проявится скрытое созвездие дома.',
          unlockTitle: 'Отголосок Оханы',
          unlockText: 'Здесь позже родится скрытое созвездие, которое не оставляет никого брошенным.',
        },
        {
          id: 'reset-point',
          title: 'Reset Point',
          kind: 'coming-soon',
          status: 'locked',
          teaser: 'Дальняя недоступная точка, которую пока можно только видеть.',
          unlockTitle: 'Точка сброса',
          unlockText: 'Она еще далеко. Эта звезда появится по-настоящему, когда весь маршрут станет живым.',
        },
